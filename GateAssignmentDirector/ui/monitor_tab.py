...
def setup_monitor_tab(parent_ui, tab):
    ...
    airport_label_text = _label(
        airport_row, text="Airport:", width=70, padx=(0, 5), pady=(0, 0), side="left"
    )
    ...
    status_label_text = _label(
        status_row, text="Status:", width=70, padx=(0, 5), pady=(0, 0), side="left"
    )
    ...
    parent_ui.override_panel = ctk.CTkFrame(
        status_frame, fg_color=c("charcoal_light"), corner_radius=6
    )
    ...
    parent_ui.override_airport_entry = ctk.CTkEntry(
        override_content,
        placeholder_text="ICAO",
        width=45,
        corner_radius=6,
        border_width=1,
    ...
    override_terminal_label = _label(
        override_content, text="Terminal:", size=12, pady=(0, 5), width=50, side="left"
    )
    attach_tooltip(override_terminal_label, "override_terminal_entry")
    parent_ui.override_terminal_entry = ctk.CTkEntry(
        override_content,
    ...
    override_gate_label = _label(
        override_content, text="Gate:", size=12, pady=(0, 5), width=30, side="left"
    )
    attach_tooltip(override_gate_label, "override_gate_entry")
    parent_ui.override_gate_entry = ctk.CTkEntry(
        override_content,
        placeholder_text="e.g., 24A",
        width=60,
        corner_radius=6,
        border_width=1,
        border_color=c("charcoal_lighter"),
    ...
    activity_label = _label(
        activity_frame, text="Recent Activity", pady=(0, 5), padx=(10, 0)
    )
